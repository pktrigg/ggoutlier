...
def reportsummary(myreport, GGOutlierlogfilename):
	...
	myreport.addparagraph("The tif file of depths are converted to point clouds. Machine learning is used to identify the signal to noise ratio in the data.  Filters are autommatically adapted to identify the 'most noisy' depths in the file. These candidates need to be evaluated against surrounding depths. The nearest neighbours are used to compute a 'regional depth surface' using a median value of the surrouding depths.  The difference in candidate-regional depth (which we call DeltaZ) is then assessed against the TVU for that depth and either flagged as an outlier or accepted as within specification. The remiaining depths are called 'outliers'.")
